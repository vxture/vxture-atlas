...
| TD-009 | `ModelGrantsPage.tsx` (vxture-platform admin portal) has no `taskProfile` form field - operators can only configure task-profile routing (TD-003b) via raw API call, not through the Admin UI | 2026-07-27 | open - not this repo's write-scope; same pattern as TD-007, both slotted into platform's atlas admin-module batch (`platform`#148 resolution); the broader "does Atlas's admin surface belong in this repo" question is answered by product_250 M-1..M-5, not open anymore |
...
| TD-012 | `model_code` is sent verbatim as the upstream provider's `model` field (`buildOpenAiCompatibleBody`, no prefix-stripping) - a `{provider_code}/{vendor_model_name}`-prefixed code (per `vxture-platform`'s `42-model-provider-registry-plan.md` §1 convention, e.g. seeded `deepseek/deepseek-chat`) 404s against the real upstream API, confirmed live with both `doubao`/`zhipu` during `vxture-atlas`#47 testing (2026-07-28) | 2026-07-28 | closed 2026-07-28 - workaround confirmed working same day (`doubao-seed-2-0-lite-260428`/`doubao-seed-2-0-pro-260215`/`glm-5.2`, bare, no prefix, all returned real `201` generations for karda's test tenant, full chain proven end to end). The underlying convention question is now answered too: `platform`#152/PR#176 clarifies `model_code` is a dispatch key, not the literal wire value, and adds `config.upstreamModel` for adapters to prefer - implemented in `resolveUpstreamModel()` (`providers/base.provider.ts`), wired into all four outbound call sites (chat/embed/rerank across doubao/zhipu/claude/private). Production's bare doubao/zhipu registrations are recorded as a reclaim item (platform's own workplan #9), not a new exception - see progress note |
...
### TD-012 - progress note (2026-07-28, closed)

- **Root cause confirmed, not just worked around**: platform's
  `42-model-provider-registry-plan.md` §1.1 (added same day, `platform`#176,
  responding to this repo's platform#152) states plainly what was previously
  implicit - `model_code` is Atlas's internal dispatch key (which adapter to
  route to), never defined as the literal value sent to an upstream vendor
  API. The two only looked identical for the three legacy unprefixed rows,
  which is exactly what masked the distinction until real Doubao/Zhipu calls
  404'd.
- **Fix**: `config.upstreamModel` (existing jsonb column on `model.models`,
  zero DDL) carries the vendor's real bare model id when it differs from
  `model_code`. `resolveUpstreamModel()` in `providers/base.provider.ts`
  reads it, falling back to `modelCode` when absent - covering both the
  legacy unprefixed rows and any model not yet given an explicit
  `upstreamModel`. Wired into all four outbound wire-body constructions
  (`buildOpenAiCompatibleBody` - chat, shared by doubao/zhipu/private -
  claude's own chat body, and zhipu's embed/rerank bodies), so there is one
  resolution point, not four copies of the same fallback logic.
- **Production's bare registrations are a recorded reclaim item, not a new
  exception**: platform's own workplan (§6 task 9) tracks renaming the
  doubao/zhipu rows back to the prefixed convention
  (`doubao/doubao-seed-2-0-lite-260428` etc.) with `config.upstreamModel` set
  to the bare vendor id, once this code was live. That data change is a
  `PUT /capability/models/:id` call through Atlas's own admin API - not a DB
  migration - and is safe to do any time after this deploy, since the
  fallback keeps working either way in the interim.
